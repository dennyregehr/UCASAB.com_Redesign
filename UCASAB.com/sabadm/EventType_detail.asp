...
' ***********************************
' Page Name: EventType_detail.asp
' Date: 3/4/2005 12:28:28 AM
...
ElseIf Request("btnSubmit") = "Delete" Or Request("fnc") = "delete" And sErr = "" Then
	...
	Else
		...
		Response.Clear: Response.Redirect "EventType_results.asp" & GetQueryString("Detail", Empty, Empty, Empty, Empty) & "&msg=" & Server.URLEncode(sMsg) & "&err=" & Server.URLEncode(sErr)
	End If
ElseIf  Request("btnDelete") = "Delete Checked Records" Then
	...
	If iRecordsDeleted = 0 Then
		...
		Response.Clear: Response.Redirect "EventType_results.asp" & GetQueryString("Detail", Empty, Empty, Empty, Empty) & "&msg=" & Server.URLEncode(sMsg)
...
If Request("fnc") <> "add" And Request("DB2ASPAction") <> "Add" Then
	...
	If iPosition = "-1" Or iPosition = "" Then
		sWhere = " WHERE " & _
		"[eventTypeID]=" & Replace(Request("PK_ieventTypeID"), "'", "''") & " "
	End If
	...
	End If
		...
		vDescription = oRS("Description")

	If Err Then
		'Response.Redirect "EventType_results.asp" & GetQueryString("Detail", Empty, Empty, Empty, Empty) & "&err=Error On Detail Page (" & Err.Description & " [" & Err.Number & "])"
...
<HEAD>
	...
	<TITLE>Event Type - Detail</TITLE>
<!-- #INCLUDE file="includes/common/DB2ASP_header.asp" -->
<FORM name=frmDetail method=post action="EventType_detail.asp<%=GetQueryString("Detail", Empty, Empty, Empty, Empty)%>">
<TABLE cellspacing=1 class=DB2ASP>
	<TR>
		<TD colspan=4 class=DB2ASPlight>
			<TABLE cellspacing=1 class=menu width=100%><TR>
			<TD class=menu>
				<SPAN class=DB2ASPtitle>Event Type</SPAN>
			</TD>
			<TD align=right class=menu width=120>
				<A href="EventType_results.asp<%=GetQueryString("Detail", Empty, Empty, Empty, Empty)%>" title="Last Results View of Records" class=DB2ASPvalue>Last Results View</A><br>
...
<TR>
	<TH align=right class=DB2ASPdark>Description</TH>
	<TD class=DB2ASPdetailcellbg>
		<INPUT type=text name="Description" SIZE=60 MAXLENGTH=100 value="<%=vDescription%>" class=DB2ASPdetailinput>
	</TD>
</TR>
...
<BR><BR>
<INPUT type=hidden name="PK_ieventTypeID" value='<%=veventTypeID%>'>
...
<% End If %>
<INPUT type=Submit name=btnBack value="Back To Results" onClick="javascript: document.location.href='EventType_results.asp<%=GetQueryString("Detail", Empty, Empty, Empty, Empty)%>'; return false;" class=DB2ASPactionbtn>&nbsp;&nbsp;
...
<!-- #INCLUDE file="includes/pages/DB2ASP_EventType_functions.asp" -->
